...
class PolyCalibrationWindow(Gtk.Window):
    def __init__(self):
        super().__init__(title="MiDaS Polynomial Calibration")
        ...
        self.points_file = os.path.join(os.path.dirname(__file__), 'calibration_points.json')
        self.snapshots_dir = os.path.join(os.path.dirname(__file__), 'calibration_snapshots')
    ...
    def setup_ui(self):
        ...
        # Alpha Frame
        f_p = Gtk.Frame(label="Alpha Calibration")
        ...
        btn_fit = Gtk.Button(label="Calculate Alpha Multiplier"); btn_fit.connect("clicked", self.on_fit); vb_p.pack_start(btn_fit,0,0,0)
        self.lbl_res = Gtk.Label(label="alpha = None"); vb_p.pack_start(self.lbl_res,0,0,0)
    ...
    def load_historical_points(self):
        try:
            if os.path.exists(self.points_file):
                ...
                if len(self.poly_data_points) >= 1:
                    self.on_fit(None)
    ...
    def on_fit(self, widget):
        if len(self.poly_data_points) < 1: return
        
        try:
            alphas = []
            for p in self.poly_data_points:
                ratio = p['M_rim'] / p['M_tray']
                alpha = (p['Z_rim'] * ratio) / p['Z_tray']
                alphas.append(alpha)
            
            final_alpha = sum(alphas) / len(alphas)
            
            self.current_coeffs = (final_alpha,)
            self.lbl_res.set_text(f"alpha = {final_alpha:.4f}")
        except Exception as e:
    ...
    def on_save(self, widget):
        ...
        # Save Alpha, but zero out a, b, c so legacy scripts don't explode
        config['alpha'] = float(self.current_coeffs[0])
        config['a'] = 0.0
        config['b'] = 0.0
        config['c'] = 0.0
...
if __name__ == "__main__":
    win = PolyCalibrationWindow(); win.show_all(); Gtk.main()
